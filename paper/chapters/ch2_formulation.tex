...
本章首先定义 \texttt{legacy\_tf81\_v1} 的固定时间离散基线：$N=30$、$t_f=81$ s。它对应原始独立的手写矩阵资产，后续不会被自动建模或参数化研究模型覆盖。第 5 章中的物理修正模型在保留相同物理常数的前提下，另行显式施加干重与区间约束，并将终端时间作为外层搜索参数；两者不能混用同一燃料结论。
...
\end{align}
该定义与代码实现~\cite{blackmore2010minimum}\cite{acikmese2007lossless}一致；参数脚本 \texttt{mars\_params.py} 以同一物理关系计算 \texttt{alpha}。
...
当6台发动机推力不等时，总推力矢量的方向可在一定范围内连续变化。定义
\begin{align}
    \bm{e}_i = [\cos\phi,\;\sin\phi\cos\psi_i,\;\sin\phi\sin\psi_i]^{\top}.
\end{align}
则总推力矢量可控空间为
\begin{align}
...
\begin{figure}[tbp]
    ...
    \begin{tikzpicture}[scale=0.9,>=stealth]
        \fill[blue!7] (0,0) -- (5.4,0.52) -- (5.4,-0.52) -- cycle;
        \draw[->] (0,0) -- (5.9,0) node[right] {$\sqrt{r_y^2+r_z^2}$（水平偏移）};
        \draw[->] (0,0) -- (0,3.6) node[above] {$r_x$（高度）};
        \draw[dashed,very thick,blue!65] (0,0) -- (5.4,0.52);
        \draw[dashed,very thick,blue!65] (0,0) -- (5.4,-0.52);
        \draw[->] (0,1.1) arc (90:84.5:1.1);
        \node[above right] at (0.55,1.02) {$\theta=86^\circ$};
        \filldraw[fill=orange!85,draw=orange!50!black] (3.5,0.30) circle (0.09);
        \node[above] at (3.5,0.38) {着陆器位置};
        \node[blue!60!black] at (4.7,-0.25) {可行安全走廊};
    \end{tikzpicture}
    \caption{下滑角约束的几何示意图}
...
\begin{enumerate}
    ...
    \item \textbf{非凸控制约束:} 式\eqref{eq:bolza_path1}中控制约束的上界依赖于状态$m(t)$，属于状态相关约束，导致可行域非凸；
