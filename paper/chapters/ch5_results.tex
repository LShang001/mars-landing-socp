...
\caption{三种 Python 求解路径的燃料消耗对比。全部收敛于 400.7 kg。}
...
C 手写版在两个编译目标下均达到约 8 ms/次的重复求解 CPU 时间，仍低于 100 ms 的典型制导周期预算。该测量从已完成的矩阵装配和 \texttt{ECOS\_setup()} 之后开始，因此不应解读为端到端重规划延迟。
...
\end{figure}值得关注的是，ECOS 标量版（8.00 ms）甚至略快于 AVX 版（8.19 ms），反快约 2.4\%。这一反直觉的现象源于 ECOS 求解的计算瓶颈特性：内点法的每次迭代中，最耗时的步骤是对 KKT 系统进行 LDL 稀疏分解，而该操作受限于稀疏矩阵的间接索引访问模式（内存带宽瓶颈），无法从 SIMD 向量化指令中获益。AVX 编译选项（\texttt{-mavx -mfma}）在此场景下非但不能加速，反而引入了额外的寄存器分配开销和指令调度延迟，从而导致性能倒退。这也表明，对于稀疏 LDL 分解主导的计算流程，进一步优化的方向应是改进稀疏矩阵的分块和缓存友好性（如超节点分解），而非增加 SIMD 指令宽度。
