...
Intel N150 平台在 Release 构建下，对已完成问题装配和初始化后的 \texttt{ECOS\_solve()} 重复计时，单次求解平均耗时约 8 ms（内点法迭代次数通常为 8\textasciitilde{}12 次）。该数字是当前代码与硬件组合的实测结果，不应直接外推为其他处理器上的执行时间。移植至 Cortex-M7 后，应先在目标板上以相同问题规模、编译选项和容差进行基准测试；表 \ref{tab:ecos_hotspots} 列出需要重点测量的计算热点。
...
DMIPS 比值、缓存结构、双精度实现和编译器代码生成均会显著影响稀疏 LDL 分解的耗时，因此不能由 N150 的 8 ms 测量值可靠推出 Cortex-M7 的微秒级结论。部署验证应记录每次 \texttt{ECOS\_solve()} 的迭代次数、最坏/分位耗时、峰值静态内存及收敛状态，并在典型与边界初始条件下重复测试。以 100 ms 为设计制导周期时，当前时间预算将 SOCP 预留为 8 ms；该预留是面向 N150 实测基线的工程预算，而非 Cortex-M7 的性能承诺。目标板若无法满足该预算，应通过缩减问题规模、预分配内存、降低重规划频率或更换计算平台解决。图 \ref{fig:timing_budget} 展示了该预算分配。
...
故障检测与重规划机制是安全关键系统的核心保障。当大气密度、风速等环境参数偏离标称值，或发动机实际推力与标称值出现偏差时，制导律必须能够在线调整。是否能在同一控制周期内完成重规划，必须以目标飞行计算机的端到端测量为准；在当前 N150 基线下，8 ms 的求解预留可纳入 100 ms 周期，但尚不能据此宣称 Cortex-M7 已满足相同实时性要求。相较于纯离线轨迹跟踪，在线重规划的鲁棒性收益仍需在目标硬件与闭环仿真中共同验证。
