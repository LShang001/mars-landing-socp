...
系数$\alpha$的物理含义为单位推力所对应的质量消耗率。根据火箭发动机的比冲定义，推力大小$\|\bm{T}_{\mathrm{total}}\|$与质量流率$\dot{m}$之间满足
\begin{align}
    \|\bm{T}_{\mathrm{total}}\| = I_{\mathrm{sp}}\,g_0\,(-\dot{m})\cos\phi,
\end{align}
其中$g_0 = 9.80665\;\mathrm{m/s^2}$为地球标准重力加速度。需要特别说明的是，式中$\cos\phi$修正项之所以出现，是因为$\dot{m}$对应的是发动机推进剂的总消耗率，而推力沿发动机轴线的分量需要乘以$\cos\phi$才能投影到$x$方向（同时沿$x$方向产生制动减速的推力分量）。然而，式\eqref{eq:dm}中$\alpha$作用于总推力幅值$\|\bm{T}_{\mathrm{total}}\|$，推导时需考虑总推力与轴向推力的区别。从推进系统能量角度推导，推进剂消耗率由所有发动机的总功率决定，与安装倾角无关，因此严格的质量消耗率为
\begin{align}
    \dot{m} = -\frac{\|\bm{T}_{\mathrm{total}}\|}{I_{\mathrm{sp}}\,g_0}.
\end{align}

对比式\eqref{eq:dm}可得$\alpha$的真实定义。在安装倾角$\phi$的配置下，若想要以等效推力矢量模型描述动力学，常用的处理方式有两种。第一种方式是将安装倾角的影响吸收进比冲参数：
\begin{align}
    \alpha = \frac{1}{I_{\mathrm{sp}}\,g_0}, \label{eq:alpha_true}
\end{align}

此时式\eqref{eq:dm}中的推力须理解为各发动机轴向推力的等效合力。为与此前文献中的约定保持一致，本文采用另一种等价形式：
\begin{align}
